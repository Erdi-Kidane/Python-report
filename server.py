...
async def saveAndDisply(s):
    logFile = open(name + ".log", "a+")
    print(s, file = logFile)
    logFile.close()

async def transmitData(message, other_name, loop):
    reader, writer = await asyncio.open_connection("127.0.0.1", 12003 + serverNames.index(other_name), loop = loop)

    await saveAndDisply("%s to %s - Send: %r" % (name, other_name, message))
    writer.write(message.encode())

    await saveAndDisply("%s to %s - Close the socket" % (name, other_name))
    writer.close()

async def evaluateString(s):
    splitted = s.split()
    try:
        print(splitted)
        if len(splitted) == 4 and splitted[0] == "IAMAT" and checkCoordinates(splitted[2]):
...
async def on_input(reader, writer):
    while not reader.at_eof():
        ...
        await saveAndDisply("%s - Received %r from %r" % (name, message, addr))

        returnMessage = await evaluateString(message) + "\n\n"
        if returnMessage != "\n\n":
            await saveAndDisply("%s - Send: %r" % (name, returnMessage))
            writer.write(returnMessage.encode())
            await writer.drain()

    await saveAndDisply("%s - Close the client socket" % name)
    writer.close()
        ...
        for other in talksWith:
            try:
                if other != from_server:
                    await transmitData("PROPAGATE "+name+" "+origin+" "+timeDiff+" "+n+" "+loc+" "+t, other, loop = loop)
            except ConnectionRefusedError:
                await saveAndDisply("%s - Could not connect to %s" % (name, other))
    else:
        await saveAndDisply("%s - Old information: %s %s %s %s %s" % (name, origin, timeDiff, n, loc, t))
...
if len(sys.argv) < 2:
    ...
    name = sys.argv[1]
    port = 12003 + serverNames.index(name)
    clients = {}
    if name == "Goloman":
    ...
    coroutine = asyncio.start_server(on_input, "127.0.0.1", port, loop = loop)
